...
const pgDatabase = process.env.PGDATABASE = 'printshop-test'
...
const createDB = require('./script/create-database')

const allProducts = require('../products.json')

tape('setup', async function (t) {
  await createDB({ database: pgDatabase, drop: true })
...
tape('should not create new product with missing fields', function (t) {
  const testProduct = {
...
tape('should not create new product with invalid tags', function (t) {
  const testProduct = {
    description: 'test description',
    imgThumb:
...
tape('should list orders', function (t) {
  const productId = 'cjv32mizj000kc9gl2r2lgj1r'
  client.listOrders({ status: 'CREATED', productId }, function (err, orders) {
    if (err) t.error('should not error')
...
tape('should edit order', function (t) {
  const productId = 'cjv32mizj000kc9gl2r2lgj1r'
  client.listOrders({ status: 'CREATED', productId }, function (err, orders) {
    if (err) t.error('should not error')
    ...
    client.editOrder(order._id, { status: 'PENDING' }, function (err, updatedOrder) {
      if (err) t.error('should not error')
      t.equal(updatedOrder.status, 'PENDING', 'status should be updated')
